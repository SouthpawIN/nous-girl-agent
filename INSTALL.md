...
- Optional: a Live2D model of your own (Nous Girl default is shipped)
...
The pet appears as a Live2D window. Right-click for menus, drag to move, double-click to chat.

## Adding a new model to the catalog

1. Edit `models/curated.yaml` (or copy from `models/suggested.yaml`)
2. Add the eikon sprite to `pet/sprites/<eikon-name>/`
...
## Running the Nous Girl agent (Hermes profile)

The agent is a Hermes profile — headless, no UI. It reads the pet's chat logs and curates notes:
...
- The pet + radio together need 1 GPU minimum (for the model). Darwin/APEX already use both 3090s.
...
- If you want the pet to run on a different GPU than training, use `CUDA_VISIBLE_DEVICES=N` before launching.
